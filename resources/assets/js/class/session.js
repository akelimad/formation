...
export default class chmSession {

  static create () {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('sessions/create')}, {
      form: {
        class: 'allInputsFormValidation',
        callback: 'chmSession.store'
      },
      width: 750,
      footer: {
        label: 'Sauvegarder'
  ...
  static edit (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('sessions/' + params.id + '/edit')}, {
      form: {
        class: 'allInputsFormValidation',
        callback: 'chmSession.store'
      },
  ...
  static show (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('sessions/' + params.id)}, {
      width: 750
    })
  }
  ...
  static store (event) {
    ...
    var ajaxParams = {
      id: id,
      type: 'POST',
      url: window.chmSite.url('sessions/store'),
      data: data,
      processData: false,
      contentType: false,
      cache: false,
  ...
  static delete (params) {
    ...
    var object = window.chmModal.show({type: 'DELETE', url: window.chmSite.url('sessions/' + params.id + '/delete'), data: {'_token': token}}, {
