...
export default class chmBudget {
  ...
  static edit (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('budgetsSession/' + params.id + '/edit')}, {
      form: {
        class: 'allInputsFormValidation',
        callback: 'chmBudget.store'
      },
  ...
  static show (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('budgetsSession/' + params.id)}, {
      width: 850
    })
  }
  ...
  static store (event) {
    ...
    var ajaxParams = {
      id: id,
      type: 'POST',
      url: window.chmSite.url('budgets/store'),
      data: data,
      processData: false,
      contentType: false,
      cache: false,
  ...
  static delete (params) {
    ...
    var object = window.chmModal.show({type: 'DELETE', url: window.chmSite.url('budgets/' + params.id + '/delete'), data: {'_token': token}}, {
