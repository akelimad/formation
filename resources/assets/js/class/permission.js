...
export default class chmPermission {
  ...
  static create () {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('utilisateurs/permissions/create')}, {
      form: {
  ...
  static edit (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('utilisateurs/permissions/' + params.id + '/edit')}, {
      form: {
        class: 'allInputsFormValidation',
        callback: 'chmPermission.store'
      },
  ...
  static store (event) {
    ...
    var ajaxParams = {
      id: id,
      type: 'POST',
      url: window.chmSite.url('utilisateurs/permissions/store'),
      data: data,
      processData: false,
      contentType: false,
      cache: false,
