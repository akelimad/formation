...
export default class chmRole {
  ...
  static create () {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('utilisateurs/roles/create')}, {
      form: {
  ...
  static edit (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('utilisateurs/roles/' + params.id + '/edit')}, {
      form: {
        class: 'allInputsFormValidation',
        callback: 'chmRole.store'
      },
  ...
  static store (event) {
    ...
    var ajaxParams = {
      id: id,
      type: 'POST',
      url: window.chmSite.url('utilisateurs/roles'),
      data: data,
      processData: false,
      contentType: false,
      cache: false,
